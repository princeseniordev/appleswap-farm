...
interface IMigratorChef {
    // Perform LP token migration to Appleswap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to AppleSwap LP tokens.
    // AppleSwap must mint EXACTLY the same amount of AppleSwap LP tokens or
    // else something bad will happen. Traditional AppleSwap does not
...
contract MasterChef is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        apple.mint(address(syrup), appleReward);
    ...
    function safeAppleTransfer(address _to, uint256 _amount) internal {
        syrup.safeAppleTransfer(_to, _amount);
    }
