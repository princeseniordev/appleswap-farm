...
contract AppleToken is BEP20('AppleToken', 'Apple') {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of APPLE
    ...
    event UpdateEmissionRate(address indexed user, uint256 ApplePerBlock);
    ...
    function pendingApple(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
            ...
            accApplePerShare = accApplePerShare.add(appleReward.mul(1e12).div(pool.lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 appleReward = multiplier.mul(ApplePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        apple.mint(devaddr, appleReward.mul(10).div(75));
        apple.mint(address(this), appleReward);
        pool.accApplePerShare = pool.accApplePerShare.add(appleReward.mul(1e12).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    //Apple has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _ApplePerBlock) external onlyOwner {
        massUpdatePools();
        ApplePerBlock = _ApplePerBlock;
